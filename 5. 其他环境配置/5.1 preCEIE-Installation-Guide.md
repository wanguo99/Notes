...
# 3. 创建一个 spack 环境，名字自己定 【以 test_env 为例】
```bash
...
# 15. 运行 fluid-openfoam 里边的 run.sh
```bash
	# 【必须】 重新开一个窗口，重新加载spack环境：
	source ~/spack/share/spack/setup-env.sh
	spack env activate test_env
